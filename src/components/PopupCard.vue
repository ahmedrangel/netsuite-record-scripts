...
const suitelet = ref<NetSuiteScript | null>();
const suitelets = ref<NetSuiteScript[]>([]);
...
const tabs = computed(() => [
  ...suitelet.value ? [{ name: "Suitelet", count: suitelets.value.length }] : [
    ...
    <div>
      ...
      <TabGroup v-else-if="fetched && !loading">
          ...
          <template v-for="(tab, i) in tabs" :key="i">
            <Tab v-slot="{ selected }" class="w-full rounded overflow-hidden cursor-pointer border border-violet-900" @click="filterInput?.focus()">
              <div class="flex align-center justify-center gap-1 p-2" :class="selected ? 'bg-violet-500/30 hover:bg-violet-500/40' : 'bg-violet-900/70 hover:bg-violet-900/82'">
                ...
                <span class="px-1.5 rounded border font-bold bg-lime-200 border-lime-600">{{ tab.count || 0 }}</span>
              </div>
            </Tab>
          </template>
        </TabList>
        <div class="flex items-center justify-between mb-1 gap-px text-xs">
          ...
          <button v-if="!isSuitelet" class="rounded-r px-2 py-1.5 bg-violet-900/70 hover:bg-violet-900/82" title="Copy record id" @click="copyToClipboard(record); copied = true;">
